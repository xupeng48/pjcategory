...
/**
 发送消息，并返回数据
 @param sel   需要执行的SEL，如果无法识别或者为NULL，则会抛出异常
 
 @param ...   参数列表，注意参数的类型必须和方法签名一致，不然会造成不确定的结果，并且struct/union不能超过256Bytes
 
 @return      对应SEL执行的结果
 
 @discussion  返回值会被NSNumber/NSValue包装
