...
#import "UIApplication+PJCategory.h"
#import "NSArray+PJCategory.h"
#import "NSObject+PJCategory.h"
...
@interface _PJUIApplicationNetworkIndicatorInfo : NSObject
...
@implementation _PJUIApplicationNetworkIndicatorInfo

@end


@implementation UIApplication (PJCategory)
...
- (BOOL)isPirated {
    ...
    if (![self _pj_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _pj_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_pj_fileExistInMainBundle:(NSString *)name {
...
PJSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _PJUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _PJUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_PJUIApplicationNetworkIndicatorInfo new];
