...
@implementation UIDevice (PJCategory)

+ (CGFloat)systemStatusBarHeight {
    // 代码里大量用了这个数据来计算绝对高度，导致inCallStatusBar会显示异常，
//    CGFloat height = 0;
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} pj_net_interface_counter;


static uint64_t pj_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t pj_net_counter_get_by_type(pj_net_interface_counter *counter, PJNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & PJNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & PJNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & PJNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & PJNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & PJNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & PJNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static pj_net_interface_counter pj_get_net_interface_counter() {
    ...
    pj_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = pj_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = pj_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(PJNetworkTrafficType)types {
    pj_net_interface_counter counter = pj_get_net_interface_counter();
    return pj_net_counter_get_by_type(&counter, types);
}
